...
app.get('/movies/', async (request, response) => {
  ...
  const movieNameArray = await db.all(moviesNameQuery)
  response.send(
...
app.get('/movies/:movieId/', async (request, response) => {
  ...
  const movie = await db.get(getMovieOnMovieId)
...
app.get('/directors/', async (request, response) => {
  const directoreQuery = `SELECT 
     * 
   FROM 
    director;`
  const directorNameArray = await db.all(directoreQuery)
  response.send(
    directorNameArray.map(eachDirector => {
      return convertDirectorDbObjectToResponseObject(eachDirector)
    }),
  )
})
